...
export default class MyApp extends App {
	...
	static async getInitialProps({ Component, ctx }) {
		...
		const repoSettings = await getRepoData(repo.GITHUB_LIMBER_SETTINGS_PATH, `parse`)
		...
		const compileDecodedCtDataToArray = Promise.all(
			_encodedCtData.map(async file => {
				...
				const _getAndDecodeFileData = await getRepoData(`/${repoSettings.config_path}/${file.name}`, `parse`)
